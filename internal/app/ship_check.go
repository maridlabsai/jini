...
func buildShipCLIHandoffDogfood() ([]shipCLIHandoffDogfood, []string, []string) {
	...
	for _, mode := range routeIDs {
		...
		if setupStatus == "ready" {
			...
			if timestampIssue := dogfoodValidationTimestampIssue(routeProof.ValidatedAt, time.Now().UTC()); timestampIssue != "" {
			...
			if releaseClaimConfig.Claims[descriptor.Mode] {
				...
				if smokeIssue := cliHandoffRecentSmokeIssue(descriptor.Mode, smokeProof, time.Now().UTC()); smokeIssue != "" {
...
const cliHandoffDogfoodFreshness = 7 * 24 * time.Hour

func dogfoodValidationTimestampIssue(value string, now time.Time) string {
	...
	if now.Sub(parsed) > cliHandoffDogfoodFreshness {
		return "recent route validation"
	}
...
const routeSmokePrompt = "Reply with exactly: jini route smoke ok"

func cliHandoffRecentSmokeIssue(routeID string, proof cliHandoffSmokeRouteProof, now time.Time) string {
	smokedAt := strings.TrimSpace(proof.SmokedAt)
	if smokedAt == "" {
		return "recent route smoke"
	}
	if !validCLIHandoffSmokeSignature(routeID, proof) {
		return "route smoke generated by jini route smoke"
	}
...
func cliHandoffSmokeKeyPath() string {
	return filepath.Join(sessionStateRoot(), "cli-smoke.key")
}
...
func saveCLIHandoffSmokeEvidence(routeID string, receipt *cliHandoffReceipt, smokedAt string) (string, error) {
	...
	key, err := loadOrCreateCLIHandoffSmokeKey()
	if err != nil {
		return cliHandoffSmokeEvidencePath(), err
	}
	...
	proof.Signature = signCLIHandoffSmokeProof(routeID, proof, key)
	payload.Routes[routeID] = proof
	data, err := json.MarshalIndent(payload, "", "  ")
	if err != nil {
		return load.Path, err
	}
...
func loadOrCreateCLIHandoffSmokeKey() ([]byte, error) {
	path := cliHandoffSmokeKeyPath()
	if key, err := readCLIHandoffSmokeKey(path); err == nil {
		return key, nil
	...
	if _, err := rand.Read(key); err != nil {
		return nil, fmt.Errorf("could not create CLI smoke signing key")
	}
...
func readCLIHandoffSmokeKey(path string) ([]byte, error) {
	data, err := os.ReadFile(path)
	if err != nil {
		return nil, err
	}
	...
	if err != nil || len(key) != 32 {
		return nil, fmt.Errorf("CLI smoke signing key is invalid: %s", filepath.Base(path))
	}
	return key, nil
}

func validCLIHandoffSmokeSignature(routeID string, proof cliHandoffSmokeRouteProof) bool {
	signature := strings.TrimSpace(proof.Signature)
	if signature == "" {
		return false
	}
	key, err := readCLIHandoffSmokeKey(cliHandoffSmokeKeyPath())
	if err != nil {
		return false
	}
	expected := signCLIHandoffSmokeProof(routeID, proof, key)
	return hmac.Equal([]byte(signature), []byte(expected))
}

func signCLIHandoffSmokeProof(routeID string, proof cliHandoffSmokeRouteProof, key []byte) string {
	mac := hmac.New(sha256.New, key)
	_, _ = io.WriteString(mac, cliHandoffSmokeSignaturePayload(routeID, proof))
	return "hmac-sha256:" + hex.EncodeToString(mac.Sum(nil))
}

func cliHandoffSmokeSignaturePayload(routeID string, proof cliHandoffSmokeRouteProof) string {
	return fmt.Sprintf(
		"v1\nroute=%s\nsmoked_at=%s\nexit_status=%d\nduration_ms=%d\nprompt_chars=%d\nstdout_chars=%d\nstderr_chars=%d\nsmoke_prompt_chars=%d\n",
		normalizeCLIHandoffRouteID(routeID),
		strings.TrimSpace(proof.SmokedAt),
		proof.ExitStatus,
		proof.DurationMS,
		proof.PromptChars,
		proof.StdoutChars,
		proof.StderrChars,
		len([]rune(routeSmokePrompt)),
	)
}
