...
func adapterRegistry() map[string]adapterDescriptor {
	return map[string]adapterDescriptor{
		...
		// BYO OpenAI-compatible providers — first-class, manually selectable
		// routes (and fallback targets). SupportsAutoRoute is false so Auto never
		// adopts a paid key on its own (frugality); the user selects them or the
		// paid Autopilot switches to them on a throttle. CostTier "byo" means the
		// escalation quote stays silent (we don't have exact per-token BYO prices,
		// so quoting frontier rates would be misleading).
		"groq": {
			ID:                "groq",
			Label:             "Groq (Llama)",
			ProviderMode:      "groq",
