...
func TestShipCheckReportsRepoValidationEvidenceAsJSON(t *testing.T) {
	...
	if !stringSliceContains(report.Next, "Run jini route smoke for each claimed CLI route.") {
		t.Fatalf("expected ship check next steps to include smoke evidence recording, got %#v", report.Next)
	}
	...
	if codexDogfood.SmokeStatus != "missing" {
		t.Fatalf("expected claimed codex to surface missing smoke status, got %#v", codexDogfood)
	}
	...
	if !stringSliceContains(codexDogfood.MissingChecks, "recent route smoke") {
...
func TestShipCheckRequiresRecentSmokeForClaimedValidatedRoute(t *testing.T) {
	repoDir := t.TempDir()
	runGitCommandForInternalTest(t, repoDir, "init")
	runGitCommandForInternalTest(t, repoDir, "config", "user.email", "test@example.com")
	runGitCommandForInternalTest(t, repoDir, "config", "user.name", "Test User")
	...
	fakeCodex := writeProviderFakeExecutable(t, fakeBin, "codex", "printf 'ok\\n'")
	stateDir := t.TempDir()
	writeCodexCLIHandoffDogfoodEvidence(t, stateDir, time.Now().UTC().Format(time.RFC3339))
	...
	exitCode := Run([]string{"check", "ship", "--format=json"}, &stdout, &stderr)
	if exitCode != 1 {
		t.Fatalf("expected claimed route without smoke evidence to block, got %d\nstdout:\n%s\nstderr:\n%s", exitCode, stdout.String(), stderr.String())
	}
	...
	if !stringSliceContains(report.Blockers, "CLI handoff dogfood missing validation for claimed route: codex") {
		t.Fatalf("expected claimed route validation blocker, got %#v", report.Blockers)
	}
	codexDogfood := shipCLIHandoffDogfoodByRoute(report.CLIHandoffDogfood, "codex")
	if codexDogfood == nil {
		t.Fatalf("expected codex dogfood row, got %#v", report.CLIHandoffDogfood)
	}
	if codexDogfood.DogfoodStatus != "needs-validation" || codexDogfood.SmokeStatus != "missing" {
		t.Fatalf("expected claimed codex to require recent smoke, got %#v", codexDogfood)
	}
	if !stringSliceContains(codexDogfood.MissingChecks, "recent route smoke") {
		t.Fatalf("expected missing smoke check, got %#v", codexDogfood.MissingChecks)
	}
}

func TestShipCheckAcceptsRecentSmokeForClaimedValidatedRoute(t *testing.T) {
	repoDir := t.TempDir()
	runGitCommandForInternalTest(t, repoDir, "init")
	runGitCommandForInternalTest(t, repoDir, "config", "user.email", "test@example.com")
	runGitCommandForInternalTest(t, repoDir, "config", "user.name", "Test User")
	...
	writeCodexCLIHandoffDogfoodEvidence(t, stateDir, validatedAt)
	writeCodexCLIHandoffSmokeEvidence(t, stateDir, smokedAt)
	...
	exitCode := Run([]string{"check", "ship", "--format=json"}, &stdout, &stderr)
	if exitCode != 0 {
		t.Fatalf("expected claimed route with dogfood and smoke evidence to pass, got %d\nstdout:\n%s\nstderr:\n%s", exitCode, stdout.String(), stderr.String())
	}
	...
	if codexDogfood.DogfoodStatus != "validated" || codexDogfood.SmokeStatus != "recent" {
		t.Fatalf("expected claimed codex to be validated with recent smoke, got %#v", codexDogfood)
	}
...
func TestShipCheckRejectsUnsignedSmokeForClaimedValidatedRoute(t *testing.T) {
	repoDir := t.TempDir()
	runGitCommandForInternalTest(t, repoDir, "init")
	runGitCommandForInternalTest(t, repoDir, "config", "user.email", "test@example.com")
	runGitCommandForInternalTest(t, repoDir, "config", "user.name", "Test User")
	...
	stateDir := t.TempDir()
	writeCodexCLIHandoffDogfoodEvidence(t, stateDir, time.Now().UTC().Format(time.RFC3339))
	writeUnsignedCodexCLIHandoffSmokeEvidence(t, stateDir, time.Now().UTC().Format(time.RFC3339))
	...
	exitCode := Run([]string{"check", "ship", "--format=json"}, &stdout, &stderr)
	if exitCode != 1 {
		t.Fatalf("expected claimed route with unsigned smoke evidence to block, got %d\nstdout:\n%s\nstderr:\n%s", exitCode, stdout.String(), stderr.String())
	}
	...
	if codexDogfood.DogfoodStatus != "needs-validation" || codexDogfood.SmokeStatus != "missing" {
		t.Fatalf("expected claimed codex to reject unsigned smoke, got %#v", codexDogfood)
	}
	if !stringSliceContains(codexDogfood.MissingChecks, "route smoke generated by jini route smoke") {
		t.Fatalf("expected unsigned smoke check, got %#v", codexDogfood.MissingChecks)
	}
}

func TestShipCheckRejectsStaleCLIHandoffDogfoodEvidenceForClaimedRoute(t *testing.T) {
	repoDir := t.TempDir()
	runGitCommandForInternalTest(t, repoDir, "init")
	runGitCommandForInternalTest(t, repoDir, "config", "user.email", "test@example.com")
	runGitCommandForInternalTest(t, repoDir, "config", "user.name", "Test User")
	...
	fakeCodex := writeProviderFakeExecutable(t, fakeBin, "codex", "printf 'ok\\n'")
	stateDir := t.TempDir()
	writeCodexCLIHandoffDogfoodEvidence(t, stateDir, time.Now().UTC().Add(-8*24*time.Hour).Format(time.RFC3339))
	writeCodexCLIHandoffSmokeEvidence(t, stateDir, time.Now().UTC().Format(time.RFC3339))
	...
	exitCode := Run([]string{"check", "ship", "--format=json"}, &stdout, &stderr)
	if exitCode != 1 {
		t.Fatalf("expected claimed route with stale dogfood evidence to block, got %d\nstdout:\n%s\nstderr:\n%s", exitCode, stdout.String(), stderr.String())
	}
	...
	if codexDogfood.DogfoodStatus != "needs-validation" {
		t.Fatalf("expected claimed codex to reject stale dogfood, got %#v", codexDogfood)
	}
	if !stringSliceContains(codexDogfood.MissingChecks, "recent route validation") {
		t.Fatalf("expected stale validation check, got %#v", codexDogfood.MissingChecks)
	}
}
...
`)
	...
	codexDogfood := shipCLIHandoffDogfoodByRoute(report.CLIHandoffDogfood, "codex")
	if codexDogfood == nil || !codexDogfood.ReleaseClaimed || codexDogfood.DogfoodStatus != "validated" || codexDogfood.SmokeStatus != "recent" {
...
func TestShipCheckTextKeepsSafePushInstructionsCompact(t *testing.T) {
	...
	for _, want := range []string{
		"Ship check blocked",
		"Branch:",
		"Blocker: CLI handoff dogfood missing validation for claimed route: codex",
		...
		"CLI handoff routes:",
		"- codex (claimed): executable ready, dogfood needs validation, smoke missing",
		...
		"Smoke evidence: .jini/cli-smoke.json for claimed routes",
...
func writeCodexCLIHandoffSmokeEvidence(t *testing.T, stateDir, smokedAt string) {
	t.Helper()
	key := make([]byte, 32)
	for i := range key {
		key[i] = 0x11
	...
	if err := os.WriteFile(filepath.Join(stateDir, "cli-smoke.key"), []byte(strings.Repeat("11", 32)+"\n"), 0o600); err != nil {
		t.Fatalf("write smoke key: %v", err)
	}
	proof := cliHandoffSmokeRouteProof{
		SmokedAt:    smokedAt,
		ExitStatus:  0,
		DurationMS:  12,
		PromptChars: 42,
	...
	proof.Signature = signCLIHandoffSmokeProof("codex", proof, key)
	writeTestFile(t, filepath.Join(stateDir, "cli-smoke.json"), `{
  "schema_version": "0.1.0",
  "context_type": "JiniCLIHandoffSmokeEvidence",
  "routes": {
    "codex": {
      "smoked_at": "`+proof.SmokedAt+`",
      "exit_status": 0,
      "duration_ms": 12,
      "prompt_chars": 42,
      "stdout_chars": 3,
...
func writeUnsignedCodexCLIHandoffSmokeEvidence(t *testing.T, stateDir, smokedAt string) {
	t.Helper()
	writeTestFile(t, filepath.Join(stateDir, "cli-smoke.json"), `{
  "schema_version": "0.1.0",
  "context_type": "JiniCLIHandoffSmokeEvidence",
...
func TestScorecardGatePassesAndExposesCompetitorPressure(t *testing.T) {
	...
	if report.PRDImplementation.TotalRequirements != 15 || report.PRDImplementation.ImplementedRequirements != 15 || report.PRDImplementation.CompletionPercent != 100 {
		t.Fatalf("expected P0 PRD implementation completion to be 15/15 = 100%%, got %#v", report.PRDImplementation)
	}
...
func TestScorecardGateTextShowsCommitGatePressure(t *testing.T) {
	...
	for _, want := range []string{
		"STATUS ok",
		"PRD IMPLEMENTATION",
		"  OK 15/15 P0 requirements implemented (100%)",
...
func TestPublishReadinessHonestAuditClaimsExposeImplementationTruth(t *testing.T) {
	...
	if !strings.Contains(cliHandoff.Evidence, "Wave 0 handoff contract") ||
		!strings.Contains(cliHandoff.Evidence, ".jini/cli-dogfood.json release checks") ||
		!strings.Contains(cliHandoff.Evidence, "signed .jini/cli-smoke.json evidence") ||
		!strings.Contains(cliHandoff.Evidence, "validation freshness") ||
		!strings.Contains(cliHandoff.Gap, "real-world dogfood") ||
		!strings.Contains(cliHandoff.Gap, "route receipt privacy") ||
		!strings.Contains(cliHandoff.Gap, "signed smoke evidence") {
