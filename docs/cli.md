...
<pre><code class="language-bash">jini</code></pre>
  ...
  <p>When there is no repo context and no existing Jini work, the fallback should stay just as calm: tell the user to run <code>jini</code> from the repo or folder that needs work, and stop there. It should not fall back to banners, start cards, mini catalogs, first-contact diagnostics, or adoption guidance.</p>
