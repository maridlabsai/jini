...
<pre><code class="language-bash">jini</code></pre>
  ...
  <p>That repo-aware start surface should stay light: one calm repo-context line, direct task suggestions, a plain list of useful commands Jini found in the repo, and only the adoption recovery a daily user actually needs.</p>
  ...
  <p>The three starter suggestions should also stay brief and action-first: <code>Review the repo and suggest the next move.</code>, <code>Fix the failing tests in this repo.</code>, and <code>Plan the change before editing.</code></p>
