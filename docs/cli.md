...
<pre><code class="language-bash">jini</code></pre>
  ...
  <p>When there is no repo context and no existing Jini work, the fallback should stay just as calm: tell the user to run <code>jini</code> from the repo or folder that needs work, point to <code>jini status /path/to/work</code> for existing Jini work, and stop there. It should not fall back to banners, start cards, mini catalogs, or first-contact diagnostics.</p>
