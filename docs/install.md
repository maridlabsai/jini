...
<div class="section-card">
  ...
  <div class="steps-grid">
    <div class="step-card">
      ...
      <p>When there is no repo context and no existing Jini work, the fallback should stay concise there too: tell the user to run <code>jini</code> from the repo or folder that needs work, point to <code>jini status /path/to/work</code> for existing Jini work, and stop there instead of surfacing diagnostics on first contact.</p>
...
jini&gt; fix failing tests
  ...
  <p>When someone types a direct task outside a repo, Jini should stay concise there too: acknowledge the task, tell them to run it from the repo that needs work, and stop there. It should not drop back to startup cards, examples, setup teaching, or adoption guidance on that path.</p>
