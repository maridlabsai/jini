...
- the renderer tracks `last_event_id` and `last_sequence` for replay after
  reconnect or sidecar restart
...
- `method` for requests
- `idempotency_key` for mutating requests
- `ok` for responses
- `event_id` and `event_type` for async events
- `sequence` for async events
- `surface: macos`

Replay rules:

- Mutating methods require `idempotency_key`.
- Go stores completed idempotency keys with action digest and result summary.
- Retrying the same key returns the same terminal response and does not repeat
  side effects.
- `app.subscribe` accepts `last_event_id` and `last_sequence`.
- Go replays missed events when still available.
- If events are no longer replayable, Go returns `projection_resync_required`
  and the renderer reloads the session projection before rendering more state.
- Cancellation is best-effort and must emit `turn.cancelled` or
  `turn.cancel_rejected` before the UI treats work as stopped.
...
- `TransientResponseVM` has no `session_id`, artifact ids, route receipt id, or
  saved-work side effect.
- It is renderable in the thread but not listed in sessions.
- It may be copied or searched in local UI history only if the user explicitly
  enables non-session history.
...
- Simple factual `compact_answer` responses use `TransientResponseVM`, not
  `TurnVM`.
...
```json
{
  ...
  "scope_options": [
    ...
    {
      "scope_id": "project_tests",
      "label": "Always allow tests in this project",
      "command_prefixes": ["go test"],
      "project_id": "proj_01",
      "ttl_seconds": 86400
    }
  ],
...
- Every approval binds `action_digest`, `target_digest`, `session_id`,
  `project_id`, route, scope, and expiry.
- Scoped approvals require explicit TTL, match rules, revocation surface, and
  audit receipt.
- A scoped approval cannot authorize commit, push, external send, publish, or
  provider escalation unless the scope explicitly names that action class.
...
```json
{
  "approval_receipt_id": "aprr_01",
  "approval_request_id": "apr_01",
  "decision": "allowed_once",
  "scope_id": "once",
  "action_digest": "sha256:...",
  "target_digest": "sha256:...",
  "decided_at": "2026-06-09T00:00:05Z",
  "consumed_at": "2026-06-09T00:00:06Z",
...
- Receipts are stored by Go with the session route evidence.
- Scoped approvals remain revocable from the Approvals inspector.
- A command whose digest does not match the approval request requires a fresh
  approval.
...
- Every event includes `event_id`, `session_id` when applicable, `surface`, and
  `emitted_at`.
- Every event includes `sequence`; sequence is monotonic per sidecar process and
  replay ordering is stable within one project/session stream.
...
```json
{
  "app_session_id": "ses_01",
  "legacy_pack_dir": ".jini/work/route-setup-help",
  "legacy_current_work_id": "example-route-setup",
  "target_session_dir": ".jini/sessions/ses_01",
  "project_id": "proj_01",
  "created_at": "2026-06-09T00:00:00Z",
...
- Go owns `SessionIdentityMap`; renderer never reads legacy state paths.
- One legacy pack dir maps to at most one `app_session_id`.
- One target session dir maps to at most one `app_session_id`.
- Opening the same work from CLI and app returns the same `app_session_id`.
- Transient simple answers never allocate an `app_session_id`.
- Migration tests must cover legacy-to-target, target-to-legacy, duplicate pack
  detection, missing current-work repair, and no-session compact answer.
...
- Phase 1 file access must name one posture: hardened-runtime-only direct
  distribution or App Sandbox with security-scoped project bookmarks
- security-scoped bookmark access, when enabled, is resolved by native shell and
  validated by Go before file operations
...
- event replay resumes from `last_event_id` and `last_sequence`
- method dispatch
- project projection
- session projection
- simple answer returns `TransientResponseVM` with no session creation
- session identity map prevents duplicate app sessions for legacy work
...
- scoped approval enforces digest, TTL, revocation, and action-class limits
...
- submit simple question returns `Paris.` as transient response
...
- retry with same idempotency key does not repeat file write or command run
