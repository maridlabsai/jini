...
> Replaced by: [number-one-platform-prd.md](../number-one-platform-prd.md) §Platform Qualities (contractual requirement language, evidence rules) and §Gates.
...
| Configured CLI handoff | implemented | Wave 0 handoff contract, Wave 1 route registry, `doctor` detection, fake downstream CLI command-shape tests, fail-closed missing/trust checks, route receipts, `check ship` setup status, signed `.jini/cli-smoke.json` evidence, recent `.jini/cli-dogfood.json` validation evidence, and freshness checks for claimed routes | Each downstream CLI still needs real-world dogfood for command templates, approvals, output shape, route receipt privacy, and signed smoke evidence on tester machines | Dogfood Wave 1 command templates against real installed CLIs without broadening the first-minute UX |
...
| P0 competitor watching | partial | `jini check competitor-watch` runtime packet, release-plan ingestion feed, scorecard deltas, stale-requirement risks, PRD, competitive release plan, readiness guard, benchmark coverage | Runtime packet and ingestion feed exist, but scheduler automation and live source refresh are not implemented | Add source-backed refresh and scheduled competitor-watch packets without changing active scope automatically |
| P0 compounding user productivity learning | implemented | `user-context.json` runtime store, `jini memory inspect/on/off/forget`, route preference signal capture, privacy filtering tests, PRD, learning-system spec, readiness guard, benchmark coverage | Runtime learning now has inspect, opt-out, forget, and safe route preference signals; broader habit signals and productivity metrics still need expansion | Expand learning signals across accepted/rejected actions and add productivity impact metrics |
