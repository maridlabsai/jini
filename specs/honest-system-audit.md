...
| Configured CLI handoff | implemented | Wave 0 handoff contract, Wave 1 route registry, `doctor` detection, fake downstream CLI smoke tests, fail-closed missing/trust checks, route receipts, `check ship` dogfood matrix | Each downstream CLI still needs real-world dogfood for command templates, approvals, output shape, and route receipt privacy | Harden Wave 1 command templates against real installed CLIs without broadening the first-minute UX |
