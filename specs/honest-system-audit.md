...
| Configured CLI handoff | implemented | Wave 0 handoff contract, Wave 1 route registry, `doctor` detection, fake downstream CLI smoke tests, fail-closed missing/trust checks, route receipts, `check ship` setup status, `.jini/cli-dogfood.json` validation evidence, and `.jini/cli-smoke.json` freshness for claimed routes | Each downstream CLI still needs real-world dogfood for command templates, approvals, output shape, route receipt privacy, and fresh smoke evidence | Harden Wave 1 command templates against real installed CLIs without broadening the first-minute UX |
