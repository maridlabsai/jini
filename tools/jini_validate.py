...
class SimpleYamlError(ValueError):
    """Raised when the internal YAML subset parser cannot understand input."""
...
def _simple_yaml_indent(line: str) -> int:
    return len(line) - len(line.lstrip(" "))


def _simple_yaml_scalar(value: str) -> Any:
    token = _strip_yaml_comment(value.strip())
    if token == "":
        return ""
    if token in {"null", "~"}:
        return None
    ...
    if token.startswith("[") or token.startswith("{"):
        try:
            return ast.literal_eval(token)
        except (SyntaxError, ValueError) as exc:  # pragma: no cover - invalid inline YAML
            raise SimpleYamlError(f"Unsupported inline YAML literal: {token}") from exc
...
def _simple_yaml_parse_inline_literal_block(
    lines: list[str],
    start: int,
    indent: int,
) -> tuple[Any, int]:
    ...
    while index < len(lines):
        ...
        if not line.strip():
            index += 1
            continue
        line_indent = _simple_yaml_indent(line)
    ...
    try:
        ...
        raise SimpleYamlError(f"Unsupported multiline YAML literal starting at line {start + 1}") from exc


def _simple_yaml_parse_block(lines: list[str], start: int, indent: int) -> tuple[Any, int]:
    index = start
    while index < len(lines):
    ...
    if index >= len(lines):
        return {}, index
    current = lines[index]
    current_indent = _simple_yaml_indent(current)
    ...
    if current.lstrip().startswith("[") or current.lstrip().startswith("{"):
        return _simple_yaml_parse_inline_literal_block(lines, index, indent)
    if current.lstrip().startswith("- "):
        return _simple_yaml_parse_list(lines, index, indent)
    return _simple_yaml_parse_dict(lines, index, indent)


def _simple_yaml_parse_block_scalar(
    lines: list[str],
    start: int,
    parent_indent: int,
    style: str,
...
def _simple_yaml_parse_list(lines: list[str], start: int, indent: int) -> tuple[list[Any], int]:
    items: list[Any] = []
    index = start
    while index < len(lines):
        line = lines[index]
        ...
        if remainder == "":
            value, index = _simple_yaml_parse_block(lines, index + 1, indent + 2)
            items.append(value)
            continue
        if remainder in {">", "|"}:
            value, index = _simple_yaml_parse_block_scalar(lines, index + 1, indent, remainder)
            items.append(value)
            continue
        ...
        if separator:
            ...
            if raw_value in {">", "|"}:
                value, next_index = _simple_yaml_parse_block_scalar(lines, index + 1, indent, raw_value)
            elif raw_value == "":
                value, next_index = _simple_yaml_parse_block(lines, index + 1, indent + 2)
            else:
            ...
            while next_index < len(lines):
                ...
                if not extra.strip():
                    next_index += 1
                    continue
                extra_indent = _simple_yaml_indent(extra)
                if extra_indent < indent + 2:
                    break
                if extra_indent == indent and extra.lstrip().startswith("- "):
                    break
                if extra_indent != indent + 2:
                    raise SimpleYamlError(f"Unexpected list item indentation at line {next_index + 1}")
                extra_key, extra_separator, extra_value = extra.strip().partition(":")
                if not extra_separator:
                    raise SimpleYamlError(f"Expected mapping entry at line {next_index + 1}")
                extra_value = extra_value.strip()
                if extra_value in {">", "|"}:
                    parsed_value, next_index = _simple_yaml_parse_block_scalar(
                        lines,
                        next_index + 1,
                        indent + 2,
                        extra_value,
                    ...
                    parsed_value, next_index = _simple_yaml_parse_block(
                        lines,
                        next_index + 1,
                        indent + 4,
                    )
                else:
                    parsed_value = _simple_yaml_scalar(extra_value)
                    next_index += 1
                mapping[extra_key.strip()] = parsed_value
            items.append(mapping)
            index = next_index
            continue
        items.append(_simple_yaml_scalar(remainder))
        index += 1
    return items, index


def _simple_yaml_parse_dict(lines: list[str], start: int, indent: int) -> tuple[dict[str, Any], int]:
    data: dict[str, Any] = {}
    index = start
    while index < len(lines):
        line = lines[index]
        ...
        if line_indent < indent:
            break
        if line_indent != indent:
            raise SimpleYamlError(f"Unexpected mapping indentation at line {index + 1}")
        stripped = line.strip()
        if stripped.startswith("- "):
            break
        key, separator, raw_value = stripped.partition(":")
        if not separator:
            raise SimpleYamlError(f"Expected mapping entry at line {index + 1}")
        key = key.strip()
        raw_value = raw_value.strip()
        if raw_value in {">", "|"}:
            value, index = _simple_yaml_parse_block_scalar(lines, index + 1, indent, raw_value)
        elif raw_value == "":
            value, index = _simple_yaml_parse_block(lines, index + 1, indent + 2)
        else:
            value = _simple_yaml_scalar(raw_value)
...
def _simple_yaml_load(text: str) -> Any:
    ...
    parsed, index = _simple_yaml_parse_block(lines, 0, 0)
    while index < len(lines):
        if lines[index].strip() and not lines[index].lstrip().startswith("#"):
            raise SimpleYamlError(f"Trailing YAML content at line {index + 1}")
        index += 1
    return parsed


def _simple_yaml_scalar_dump(value: Any) -> str:
    if value is None:
        return "null"
    if value is True:
        return "true"
    ...
    if isinstance(value, dict):
        ...
        for key, item in value.items():
            if isinstance(item, (dict, list)):
                ...
                lines.extend(_simple_yaml_dump_lines(item, indent + 2))
            else:
                lines.append(f"{prefix}{key}: {_simple_yaml_scalar_dump(item)}")
        ...
        for item in value:
            if isinstance(item, dict) and item:
                ...
                if isinstance(first_value, (dict, list)):
                    ...
                    lines.extend(_simple_yaml_dump_lines(first_value, indent + 4))
                else:
                    lines.append(f"{prefix}- {first_key}: {_simple_yaml_scalar_dump(first_value)}")
                for key, nested_value in iterator:
                    if isinstance(nested_value, (dict, list)):
                        lines.append(f"{prefix}  {key}:")
                        lines.extend(_simple_yaml_dump_lines(nested_value, indent + 4))
                    else:
                        lines.append(f"{prefix}  {key}: {_simple_yaml_scalar_dump(nested_value)}")
            elif isinstance(item, list):
                lines.append(f"{prefix}-")
                lines.extend(_simple_yaml_dump_lines(item, indent + 2))
            else:
                lines.append(f"{prefix}- {_simple_yaml_scalar_dump(item)}")
        return lines or [f"{prefix}[]"]
    return [f"{prefix}{_simple_yaml_scalar_dump(value)}"]
...
def dump_document(path: Path, data: Any) -> None:
    if path.suffix.lower() == ".json":
        ...
        return
    path.write_text(_yaml_dump_text(data), encoding="utf-8")
...
def build_publish_readiness() -> dict[str, Any]:
    ...
    doc_paths = [
        ...
        ROOT / "specs" / "workstream-technical-framework.md",
        ROOT / "specs" / "workstream-technical-framework-review.md",
                ...
                *lean_platform_gate["checks"],
                {
                    ...
                    "markers": [
                        ...
                        "route_feedback_health:tracked",
                        "route_feedback_impact:measured-or-empty",
...
def parse_markdown_front_matter(path: Path) -> tuple[dict[str, Any], str]:
    ...
    body = parts[1]
    try:
        meta = _yaml_load_text(raw_meta) or {}
...
def bootstrap_workspace_steering(workspace_root: Path) -> dict[str, Any]:
    ...
    def write_doc(filename: str, metadata: dict[str, Any], lines: list[str]) -> Path:
        ...
        payload = "---\n" + _yaml_dump_text(metadata).strip() + "\n---\n\n" + "\n".join(lines) + "\n"
...
def print_cli_overview() -> None:
    ...
    print("Finish work, keep the next step visible, and resume without rebuilding context.")
    print()
    print("OPEN JINI")
    print(f"  {cli}")
    print()
