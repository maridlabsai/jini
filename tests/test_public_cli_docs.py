...
class PublicCliDocsTests(unittest.TestCase):
    ...
    def test_cli_guide_promotes_commands_as_public_catalog(self) -> None:
        ...
        for marker in (
            ...
            "That repo-aware start surface should stay light: skip the generic empty-state sentence, show one calm repo-context line, let the direct task suggestions stand on their own, use a soft cue like <code>Useful here:</code> above one or two useful commands Jini found in the repo, and keep at most one quiet adoption hint for existing Jini work.",
            ...
            "When there is no repo context and no existing Jini work, the fallback should stay just as calm",
            "interactive <code>jini</code> should still open the live shell",
            "Run this from the repo or folder that needs work.",
            "stay open instead of exiting",
            "Non-interactive fallback output should stay just as calm",
            "drop the version banner, repo receipt, active-work receipt, and startup coaching line",
            "That same calm shell shape should appear when Jini resumes remembered work too",
            "skip the full outcome report before the prompt",
            "jini&gt; fix failing tests",
            "Working on: fix failing tests",
            "Start with `make test`.",
            "jini&gt; doctor",
            "DOCTOR   Local preview [ok]",
            "keep the <code>jini&gt;</code> prompt open",
            "keep the controls in the background instead of teaching them before the task starts",
            "<code>commands</code>, <code>doctor</code>, <code>admin help</code>, and <code>exit</code> should still work inside the same live session, but they should answer with concise in-shell summaries instead of relaunching the full catalog or operator cards.",
    ...
    def test_install_page_uses_same_public_vs_admin_boundary(self) -> None:
        ...
        for marker in (
            ...
            "<code>status</code>, <code>continue</code>, <code>open</code>, and <code>doctor</code>",
            "When someone types a direct task outside a repo, Jini should stay concise there too: acknowledge the task, tell them to run it from the repo or folder that needs work, and stop there.",
            "adoption guidance on that path",
            "Help surfaces and <code>jini commands</code> are catalogs, not request entrypoints.",
            ...
            "Fix the failing tests in this repo.",
            "Review the current branch and call out risks.",
            "When there is no repo context and no existing Jini work, the fallback should stay concise there too",
            "interactive <code>jini</code> should still open the live shell there too",
            "Run this from the repo or folder that needs work.",
            "stay open instead of exiting",
            "Non-interactive fallback output should stay concise",
            "drop the version banner, repo receipt, active-work receipt, and startup coaching line",
            "That same calm shell shape should appear when Jini resumes remembered work too",
            "skip the full outcome report before the prompt",
            "jini&gt;</code> prompt",
            "The task should stay primary, and the controls should stay in the background until you need them.",
            "<code>commands</code>, <code>doctor</code>, <code>admin help</code>, and <code>exit</code> should still work as in-session escape hatches instead of forcing a relaunch, but they should answer with concise in-shell summaries instead of dumping the full catalog or operator cards.",
