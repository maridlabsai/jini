...
class PublicCliDocsTests(unittest.TestCase):
    ...
    def test_cli_guide_promotes_commands_as_public_catalog(self) -> None:
        ...
        for marker in (
            ...
            "run <code>jini</code> from the repo or folder that needs work",
            "<code>jini status /path/to/work</code> for existing Jini work",
            "mention <code>jini doctor</code> only as a blocked-execution recovery path",
    ...
    def test_install_page_uses_same_public_vs_admin_boundary(self) -> None:
        ...
        for marker in (
            ...
            "<code>status</code>, <code>continue</code>, <code>open</code>, and <code>doctor</code>",
            "When someone types a direct task outside a repo, Jini should stay concise there too: acknowledge the task, then tell them to run it from a repo or adopt existing Jini work.",
            "Help surfaces and <code>jini commands</code> are catalogs, not request entrypoints.",
            ...
            "run <code>jini</code> from the repo or folder that needs work",
            "point to <code>jini status /path/to/work</code> for existing Jini work",
            "mention <code>jini doctor</code> only when execution is actually blocked",
            "drop the version banner, repo receipt, active-work receipt, and startup coaching line",
            "That same calm shell shape should appear when Jini resumes remembered work too",
            "skip the full outcome report before the prompt",
            "jini&gt;</code> prompt",
