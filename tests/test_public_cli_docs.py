...
class PublicCliDocsTests(unittest.TestCase):
    ...
    def test_cli_guide_promotes_commands_as_public_catalog(self) -> None:
        ...
        for marker in (
            ...
            "That repo-aware start surface should stay light: one calm repo-context line, direct task suggestions, a plain list of useful commands Jini found in the repo, and only the adoption recovery a daily user actually needs.",
            "Internal diagnostics like <code>repo-map</code> and setup surfaces like <code>doctor</code> should stay off the first screen.",
            "A Claude Code, Codex, or GitHub CLI user should see task suggestions first, then only <code>status /path/to/work</code> when an existing Jini work path actually needs adoption.",
