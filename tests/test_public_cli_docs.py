...
class PublicCliDocsTests(unittest.TestCase):
    ...
    def test_cli_guide_promotes_commands_as_public_catalog(self) -> None:
        ...
        for marker in (
            ...
            "That repo-aware start surface should stay light: one calm repo-context line, direct task suggestions, detected entrypoints, and only the recovery commands a daily user actually needs.",
            "Internal diagnostics like <code>repo-map</code> should stay off the first screen.",
            "A Claude Code, Codex, or GitHub CLI user should see task suggestions first, then only <code>status /path/to/work</code> or <code>doctor</code> if adoption or setup is actually relevant.",
