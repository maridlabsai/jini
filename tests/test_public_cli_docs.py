...
class PublicCliDocsTests(unittest.TestCase):
    ...
    def test_cli_guide_promotes_commands_as_public_catalog(self) -> None:
        ...
        for marker in (
            ...
            "That repo-aware start surface should stay light: skip the generic empty-state sentence, show one calm repo-context line, offer direct task suggestions, use a soft cue like <code>Useful here:</code> above one or two useful commands Jini found in the repo, and keep at most one quiet adoption hint for existing Jini work.",
