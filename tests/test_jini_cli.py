...
class JiniCliConformanceTests(unittest.TestCase):
    ...
    def assert_compact_interactive_escape_hatch_surface(self, stdout: str) -> None:
        ...
        self.assertIn("IN-SHELL  commands  doctor  admin help  exit", stdout)
        ...
        self.assertIn("MORE     run `jini admin help` for the full inventory", stdout)
    ...
    def assert_interactive_multi_token_recovery_hints(self, result: subprocess.CompletedProcess[str]) -> None:
        ...
        self.assertIn("Use `admin help` by itself inside the live shell.", result.stderr)
        self.assertNotIn("usage: jini", result.stderr)
    ...
    def assert_interactive_help_alias_surface(self, result: subprocess.CompletedProcess[str]) -> None:
        ...
        self.assertIn("IN-SHELL  commands  doctor  admin help  exit", result.stdout)
    ...
    def test_help_shows_curated_beginner_surface(self) -> None:
        ...
        self.assert_ok(result)
        self.assertIn("Jini", result.stdout)
        self.assertIn("Paste what you want finished.", result.stdout)
        self.assertIn("Examples:", result.stdout)
        self.assertIn("Turn meeting notes into something I can send", result.stdout)
        self.assertIn("Check whether a plan is ready to hand off", result.stdout)
        self.assertIn("Plan a 7 day Paris trip for two adults in October", result.stdout)
        self.assertIn("Compare these vendors and recommend one", result.stdout)
        self.assertIn("Nothing will be sent yet.", result.stdout)
        self.assertIn("If you want help shaping a messy ask, type `I'm not sure`.", result.stdout)
        self.assertIn("If you need commands, type `help`.", result.stdout)
        self.assertNotIn("SUPPORT COMMANDS", result.stdout)
        self.assertNotIn("jini status", result.stdout)
        self.assertNotIn("jini continue", result.stdout)
        self.assertNotIn("jini open", result.stdout)
        self.assertNotIn("jini doctor", result.stdout)
        self.assertNotIn("jini admin help", result.stdout)
    ...
    def test_help_all_shows_public_command_inventory(self) -> None:
        ...
        self.assertIn("jini admin help", result.stdout)
        self.assertNotIn("jini try-example research-prd", result.stdout)
    ...
    def test_runtime_readouts_keep_unhealthy_explicit_pin_with_local_route_evidence(self) -> None:
        ...
        zepto_resume_result = self.run_cli(
            "resume",
            pack_dir,
            "--runtime-target",
            "kiro-cli",
        ...
        self.assert_ok(zepto_resume_result)
        zepto_resume_payload = json.loads(zepto_resume_result.stdout)
        self.assertEqual("local-workhorse", zepto_resume_payload["runtime_readout"]["route"])
        self.assertEqual("kiro-cli", zepto_resume_payload["runtime_target"]["selected"])
        self.assertTrue(
            zepto_resume_payload["runtime_readout"]["reason"].startswith(
                "State `operational` requires stronger"
            )
        )
